...
class DCT(nn.Module):
    def __init__(self, N=8, in_channal=3, num_channels=192):
        ...
        self.dct_conv.weight.data = torch.cat([self.weight] * self.in_channal, dim=0)
...
if __name__ == '__main__':
    # img_bgr = cv2.imread("F:/datasets/aircraft/justonejpg/plsn/plane.jpg")
    # img_bgr = cv2.resize(img_bgr, (448, 448))
    # img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    # img_ycrcb = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2YCrCb).transpose(2, 0, 1)
    # img_ycrcb = np.array(img_ycrcb, dtype="float64")
    # img_ycrcb = torch.from_numpy(img_ycrcb)
    # img_ycrcb = torch.unsqueeze(img_ycrcb, dim=0)
    #
    # DCT = DCT(choose_frequency=63)
    # dct_img = DCT(img_ycrcb)
    # print(dct_img.shape)
    # dct_img = dct_img[0, :, :, :]
    # dct_img = np.array(dct_img, dtype="uint8").transpose(1, 2, 0)
    # dct_img_bgr = cv2.cvtColor(dct_img, cv2.COLOR_YCrCb2BGR)
    # dct_img_y = dct_img[:, :, 0]
    # dct_img_cr = dct_img[:, :, 1]
    # dct_img_cb = dct_img[:, :, 2]
    #
    # cv2.namedWindow('plane', cv2.WINDOW_NORMAL)
    # cv2.imshow('plane', img_ycrcb)
    # cv2.waitKey(0)
    data = torch.rand(1, 3, 448, 448)
    dct = DCT(num_channels=192)
    Y = dct(data)
    print(Y.shape)
