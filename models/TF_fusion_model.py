...
class UpScaleDctResnet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, bn_threshold=2e-2):
        ...
        # training parameters
        self.alpha_beta = nn.Parameter(torch.ones(2, requires_grad=True))
        self.register_parameter('alpha_beta', self.alpha_beta)
    ...
    def forward(self, x, dct_img):
        ...
        alpha_beta = F.softmax(self.alpha_beta, dim=0)
        out_t_f = alpha_beta[0] * out_t.detach() + alpha_beta[1] * out_f.detach()
        return [out_t, out_f, out_t_f], alpha_beta


def UpScaleResnet9(num_classes, threshold):
    return UpScaleDctResnet(BasicBlockFusion, [1, 1, 1, 1], num_classes=num_classes, bn_threshold=threshold)


def UpScaleResnet18(num_classes, threshold):
    return UpScaleDctResnet(BasicBlockFusion, [2, 2, 2, 2], num_classes=num_classes, bn_threshold=threshold)


def UpScaleResnet34(num_classes, threshold):
    return UpScaleDctResnet(BasicBlockFusion, [3, 4, 6, 3], num_classes=num_classes, bn_threshold=threshold)


def UpScaleResnet50(num_classes, threshold):
    return UpScaleDctResnet(BottleneckFusion, [3, 4, 6, 3], num_classes=num_classes, bn_threshold=threshold)


def UpScaleResnet101(num_classes, threshold):
    return UpScaleDctResnet(BottleneckFusion, [3, 4, 23, 3], num_classes=num_classes, bn_threshold=threshold)


def UpScaleResnet152(num_classes, threshold):
    return UpScaleDctResnet(BottleneckFusion, [3, 8, 36, 3], num_classes=num_classes, bn_threshold=threshold)
