...
def parse_args():
    ...
    parser.add_argument('--dataset', metavar='DIR', default='bird', help='aircraft bird car')
    parser.add_argument('--lr-begin', default=0.00125, type=float, metavar='LR', help='initial learning rate')
    parser.add_argument('--epochs', default=500, type=int, metavar='N',
                        help='number of total epochs to run, if learning from scratch, epochs >= 500')
    parser.add_argument('--gamma', default=0.02, type=float, metavar='M', help='gamma')
    parser.add_argument('-b', '--batch-size', default=6, type=int, metavar='N', help='mini-batch size')
    parser.add_argument('--langbuda', default=2e-6, type=float, help='Hyper-parameter langbuda')
    parser.add_argument('--num-workers', default=4, type=int, metavar='N', help='num_workers')
    parser.add_argument('--num-channels', default=64, type=int, metavar='N', help='num_channels')
    parser.add_argument('--pre-trained', default=False, type=bool, help='load pre-trained model')
    parser.add_argument('--use-penalty', default=False, type=bool, help='use penalty')
    parser.add_argument('--store-pth', default='./parameters/bird/TF', type=str, metavar='store_para_path',
                        help='path to save para of the model')
...
# dataset
# Data augmentation, only use horizon flip
...
if args.dataset == "aircraft":
    train_data = AircraftDataset(config.root_of_aircraft + "train", args.num_channels, transform_448=train_transform,
                                 train=True)
    test_data = AircraftDataset(config.root_of_aircraft + "test", args.num_channels, transform_448=test_transform,
                                train=False)
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    test_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    # model(classes)
    net = UpScaleResnet50(100, args.gamma)
    net.cuda()

elif args.dataset == "bird":
    train_data = BirdsDataset(config.root_of_bird + "train", args.num_channels, transform_448=train_transform,
                              train=True)
    test_data = BirdsDataset(config.root_of_bird + "test", args.num_channels, transform_448=test_transform,
                             train=False)
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    test_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    # model
    net = UpScaleResnet50(200, args.gamma)
    net.cuda()

elif args.dataset == "car":
    train_data = CarsDataset(config.root_of_car + "train", args.num_channels, transform_448=train_transform,
                             train=True)
    test_data = CarsDataset(config.root_of_car + "test", args.num_channels, transform_448=test_transform,
                            train=False)
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    test_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    # model
    net = UpScaleResnet50(196, args.gamma)
    net.cuda()

# Whether or load the pre-trained model
if args.pre_trained:
...
if __name__ == "__main__":
    print("Training with para: batch-size:{}, num-workers:{}, langbuda:{}, gamma:{}".format(args.batch_size, args.num_workers, args.langbuda, args.gamma))
        ...
        for batch_idx, (inputs, dcts, targets) in enumerate(tqdm(train_loader, ncols=80)):
            ...
            if args.use_penalty:
                for output in outputs:
                    ...
                    L1_norm = sum([L1_penalty(m).cuda() for m in L1_params])  # L1 penalty
