...
from YCrCb_DCT_transformation.DCT import DCTRecombination, rgb2ycrcb
...
# Make label
labels_dict = []
for root, dirs, _ in os.walk(root_of_aircraft + "train"):
...
class AircraftDataset(Dataset):
    def __init__(self, data_dir, num_channels, transform_448=None, train=True):
        ...
        self.DCT = DCTRecombination(N=8, in_channal=3, num_channels=num_channels)  # DCT transformation
        self.train = train
    ...
    def __getitem__(self, index):
        ...
        img_448_ycrcb_tensor = torch.unsqueeze(img_448_ycrcb, dim=0)
        dct_img = self.DCT(img_448_ycrcb_tensor).squeeze()
        if self.train:
            if self.num_channels == 192:
                dct_img = transforms.Normalize(mean_train_dct_192_air, std_train_dct_192_air)(dct_img)
            if self.num_channels == 64:
                dct_img = transforms.Normalize(mean_train_dct_64_air, std_train_dct_64_air)(dct_img)
        else:
            if self.num_channels == 192:
                dct_img = transforms.Normalize(mean_test_dct_192_air, std_test_dct_192_air)(dct_img)
            if self.num_channels == 64:
                dct_img = transforms.Normalize(mean_test_dct_64_air, std_test_dct_64_air)(dct_img)
